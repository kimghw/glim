...
"""
Ouster 라이다 드라이버 실행 Launch 파일
센서 IP: 192.168.10.10 (기본값)
"""
...
def generate_launch_description():
    ...
    # 현재 패키지의 config 디렉토리 경로
    current_dir = Path(__file__).parent.parent
    default_params_file = current_dir / 'config' / 'ouster_params.yaml'
    ...
    # Sensor Finalized Event
    sensor_finalized_event = RegisterEventHandler(
        OnStateTransition(
            target_lifecycle_node=os_driver,
            goal_state='finalized',
            entities=[
                LogInfo(msg="Failed to communicate with the sensor."),
